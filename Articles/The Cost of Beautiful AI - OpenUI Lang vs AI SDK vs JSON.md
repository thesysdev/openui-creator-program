...
![Official OpenUI benchmark token totals](../assets/openui-token-cost-official-benchmark.svg)
...
That also gives a simple price sensitivity table. If the output-token price is:

| Output-token price | OpenUI Lang | Vercel JSON-Render | Monthly savings |
| --- | ---: | ---: | ---: |
| $1 per 1M output tokens | $68.60 | $145.40 | $76.80 |
| $5 per 1M output tokens | $343.00 | $727.00 | $384.00 |
| $10 per 1M output tokens | $686.00 | $1,454.00 | $768.00 |

The point is not the exact provider price. Model pricing changes. The durable point is the ratio: with this benchmark mix, the JSON-patch-style representation spends about 2.1x as many output tokens as OpenUI Lang to describe the same interfaces.
